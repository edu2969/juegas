...
Template.desafioestudiante.rendered = () => {
  ...
  $("#input-hasta").datetimepicker({
    locale: moment.locale("es"),
    format: "DD/MM/YYYY HH:mm",
    defaultDate: moment().startOf("day").add(44, "hour"),
  });
};
...
Template.desafioestudiante.helpers({
  ...
  capsula() {
    const desafio = Session.get("DesafioSeleccionado");
    if (!desafio) return;
    let capsula = VideosCapsulas.findOne({
      $or: [{ "meta.desafioId": desafio._id }, { "meta.pendiente": true }],
    });
...
Template.desafioestudiante.events({
  "click .contenedor-desafio-estudiante .cruz"() {
    ...
    document
      .querySelector(".contenedor-desafio-estudiante")
  ...
  "click .btn-guardar-desafio"() {
    ...
    document.querySelectorAll(".campo").forEach(function (item) {
      ...
      const esFecha =
        item.classList.value.indexOf("datetimepicker-component") != -1;
      let valor = esFecha
        ? moment(item.value, "DD/MM/YYYY HH:mm").toDate()
        : item.value;

      if (
        (!esFecha && anterior !== valor) ||
    ...
    let tipoEvidencias = [];
    document
      .querySelectorAll("input[name='tipo-evidencia']")
      .forEach((item) => {
        const value = item.value;
        const checked = item.checked;
        if (checked) {
          tipoEvidencias.push(value);
        }
      });
    doc.tipoEvidencias = tipoEvidencias;
